...
fun DisplayStatus(viewModel: LoginViewModel) {
    ...
    when (val status = viewModel.uiState) {
        ...
        is LoginViewModel.LoginUiState.AutoLogin -> {
            Card(
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(top = 16.dp),
                colors = CardDefaults.cardColors(
                    containerColor = LightBlueStatusBackground.copy(alpha = 0.9f)
                )
            ) {
                Row(
                    modifier = Modifier
                    ...
                    Icon(
                        ...
                        tint = LightBlueStatusContent
                    )
                    Spacer(modifier = Modifier.width(16.dp))
                    Text(
                        status.message,
                        style = MaterialTheme.typography.bodyLarge,
                        color = LightBlueStatusContent
                    )
                }
            }
        }
